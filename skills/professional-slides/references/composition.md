...
Attach a process step's actor, input, output and failure route to that step. A detached list of controls does not show where they operate. Decision branches represent alternative conditions; sequential actions within one route must not become sibling branches. Walk a named case through the diagram to check both relationships before rendering.
