...
REVIEWER_POLICY = json.loads((SKILL_ROOT / "references/evaluation/rules.json").read_text())["reviewer"]
SKILL_REFERENCE_MAP = {
    "contract": (
...
def summary_purpose_exception(contract: dict[str, Any]) -> bool:
    decision = contract.get("executiveSummaryDecision")
    return isinstance(decision, dict) and decision.get("status") == "not_required" and decision.get("purpose") in {"explanation", "workshop", "pitch"} and non_empty_string(decision.get("rationale"))


def validate_synthesis_metadata(contract: dict[str, Any], slide_by_number: dict[int, Any]) -> list[str]:
    """Validate optional planning/provenance metadata, without requiring a summary form."""
    errors: list[str] = []
    groups = contract.get("synthesisGroups", [])
    if not isinstance(groups, list):
        errors.append("synthesisGroups must be a list")
        groups = []
    seen = set()
    for index, group in enumerate(groups):
        loc = f"synthesisGroups[{index}]"
        if not isinstance(group, dict):
            errors.append(f"{loc} must be an object")
            continue
        gid = group.get("id")
        if not non_empty_string(gid) or gid in seen:
            errors.append(f"{loc}.id must be a unique non-empty string")
        else:
            seen.add(gid)
        if group.get("scope") not in {"deck", "workstream", "diagnostic", "chapter", "teaching", "closing"}:
            errors.append(f"{loc}.scope is invalid")
        if group.get("decisionStage") not in {"diagnosis", "options", "recommendation", "endorsement", "authorization", "explanation"}:
            errors.append(f"{loc}.decisionStage is invalid")
        if not non_empty_string(group.get("governingAnswer")):
            errors.append(f"{loc}.governingAnswer must be a non-empty string")
        for key in ("slides", "proofSlides"):
            values = group.get(key, [] if key == "proofSlides" else None)
            if not isinstance(values, list) or (key == "slides" and not values) or any(isinstance(v, bool) or not isinstance(v, int) or v not in slide_by_number for v in values):
                errors.append(f"{loc}.{key} must reference existing slide numbers")
            elif len(set(values)) != len(values) or values != sorted(values):
                errors.append(f"{loc}.{key} must be unique and in slide order")
        conditions = group.get("conditions", [])
        if not isinstance(conditions, list) or not all(non_empty_string(v) for v in conditions):
            errors.append(f"{loc}.conditions must be a list of substantive strings")
    sources = contract.get("sourceDocuments", [])
    if not isinstance(sources, list):
        errors.append("sourceDocuments must be a list")
        sources = []
    seen_sources = set()
    for index, source in enumerate(sources):
        loc = f"sourceDocuments[{index}]"
        if not isinstance(source, dict):
            errors.append(f"{loc} must be an object")
            continue
        sid = source.get("id")
        if not non_empty_string(sid) or sid in seen_sources:
            errors.append(f"{loc}.id must be a unique non-empty string")
        else:
            seen_sources.add(sid)
        fmt = source.get("format")
        if fmt not in {"pdf", "pptx", "text"}:
            errors.append(f"{loc}.format must be pdf, pptx, or text")
        if not non_empty_string(source.get("form")):
            errors.append(f"{loc}.form must be a non-empty string")
        for key, allowed in (("accessStatus", {"available", "inaccessible", "missing"}), ("identityStatus", {"verified", "provisional", "mismatch"}), ("reviewStatus", {"not-reviewed", "reviewed", "uncertain"})):
            if source.get(key) not in allowed:
                errors.append(f"{loc}.{key} is invalid")
        if source.get("accessStatus") == "available":
            if not non_empty_string(source.get("path")):
                errors.append(f"{loc}.path is required for available sources")
            if not isinstance(source.get("sha256"), str) or not re.fullmatch(r"[0-9a-fA-F]{64}", source["sha256"]):
                errors.append(f"{loc}.sha256 must identify the available source bytes")
        for count_key in ("pageCount", "slideCount"):
            if count_key in source and (isinstance(source[count_key], bool) or not isinstance(source[count_key], int) or source[count_key] < 1):
                errors.append(f"{loc}.{count_key} must be a positive integer")
        ranges = source.get("reviewedRanges", [])
        if not isinstance(ranges, list):
            errors.append(f"{loc}.reviewedRanges must be a list")
            continue
        for ri, span in enumerate(ranges):
            rloc = f"{loc}.reviewedRanges[{ri}]"
            if not isinstance(span, dict):
                errors.append(f"{rloc} must be an object")
                continue
            expected = {"pdf": "physical-pdf-page", "pptx": "native-slide"}.get(fmt)
            if expected is None or span.get("numbering") != expected:
                errors.append(f"{rloc}.numbering must match the source format; Office reflow pages are not native slides")
            start, end = span.get("start"), span.get("end")
            if any(isinstance(v, bool) or not isinstance(v, int) or v < 1 for v in (start, end)) or start > end:
                errors.append(f"{rloc} must have positive ordered start/end bounds")
            else:
                bound = source.get("slideCount" if fmt == "pptx" else "pageCount")
                if isinstance(bound, int) and end > bound:
                    errors.append(f"{rloc} exceeds source bounds")
    return errors


def validate_production_story(spec: dict[str, Any]) -> list[str]:
    """Reuse canonical synthesis validation for the portable spec without legacy file scaffolding."""
    context = spec.get("context", {})
    slides = spec.get("deckPlan", {}).get("slides", [])
    errors = validate_synthesis_metadata(context, {i + 1: s for i, s in enumerate(slides)})
    if not str(spec.get("schema", "")).endswith("/v2"):
        return errors
    decision = context.get("executiveSummaryDecision", {})
    if decision.get("status") not in {"required", "not_required", "present"} or not non_empty_string(decision.get("rationale")):
        errors.append("Production summary decision needs status and rationale")
    stage = context.get("decisionStage")
    if stage not in {"diagnosis", "options", "recommendation", "endorsement", "authorization", "explanation"}:
        errors.append("Production decisionStage is required")
    if decision.get("status") == "not_required" and not summary_purpose_exception(context):
        errors.append("Summary exception must use canonical purpose policy")
    groups = context.get("synthesisGroups", [])
    if decision.get("status") in {"required", "present"} and not groups:
        errors.append("Required synthesis needs an explicit ordered group")
    for group in groups:
        if group.get("decisionStage") != stage:
            errors.append("Synthesis decision stage differs from declared deck authority")
        branches = group.get("branches", [])
        if not branches:
            errors.append("Synthesis requires branch coverage")
        seen = set()
        for branch in branches:
            if not isinstance(branch, dict) or not non_empty_string(branch.get("id")) or branch.get("id") in seen:
                errors.append("Synthesis branch IDs must be unique")
                continue
            seen.add(branch["id"])
            members = branch.get("slides", [])
            if not members or any(v not in group.get("slides", []) for v in members):
                errors.append("Branch coverage must belong to its synthesis sequence")
            if not branch.get("evidence") or any(e not in {r.get("id") for r in spec.get("evidence", [])} for e in branch["evidence"]):
                errors.append("Synthesis branch needs known evidence")
        if len(group.get("slides", [])) > 1 and not non_empty_string(group.get("transition")):
            errors.append("Multi-page synthesis requires a transition into detail")
    return errors


def synthesis_item_ids(items: Any) -> set[str]:
    """Read exact authoring IDs, including nested composition items."""
    ids: set[str] = set()
    if isinstance(items, list):
        for item in items:
            ids.update(synthesis_item_ids(item))
    elif isinstance(items, dict):
        if non_empty_string(items.get("id")):
            ids.add(items["id"])
        for key in ("items", "children"):
            ids.update(synthesis_item_ids(items.get(key)))
    return ids


def required_opening(contract: dict[str, Any]) -> bool:
    if contract.get("workflowMode") != "new_deck" or summary_purpose_exception(contract):
        return False
...
def validate_contract(contract: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(summary, dict):
        ...
        if "purpose" in summary and summary["purpose"] not in {"decision", "explanation", "workshop", "pitch"}:
            errors.append("executiveSummaryDecision.purpose is invalid")
    ...
    for index, slide in enumerate(slides):
        ...
        if slide.get("pageType") == "executive_synthesis":
            ...
            if not isinstance(synthesis, dict):
                ...
                for field in ("answer",) + (("overallAction",) if "overallAction" in synthesis else ()):
                ...
                item_ids = synthesis.get("itemIds")
                if item_ids is not None:
                    known_ids = synthesis_item_ids(slide.get("items"))
                    if not isinstance(item_ids, list) or not item_ids or not all(non_empty_string(v) for v in item_ids):
                        errors.append(f"{location}.executiveSynthesis.itemIds must be non-empty exact authoring IDs")
                    elif len(set(item_ids)) != len(item_ids) or any(v not in known_ids for v in item_ids):
                        errors.append(f"{location}.executiveSynthesis.itemIds must be unique references to slide.items")
                if branches is None and item_ids is not None:
                    pass
                elif not isinstance(branches, list) or not branches:
                    errors.append(f"{location}.executiveSynthesis.branches must contain substantive branches, or use itemIds")
                else:
                    for branch_index, branch in enumerate(branches):
                        ...
                        if synthesis.get("format") == "thematic-bullets":
                            ...
                            if not isinstance(bullets, list) or not bullets or not all(isinstance(b, str) and b.strip() for b in bullets):
                                errors.append(f"{branch_location} needs substantive bullets")
    ...
    errors.extend(validate_synthesis_metadata(contract, slide_by_number))
    ...
    if required_opening(contract):
        ...
        if not has_summary and not summary_purpose_exception(contract):
...
def validate_manifest(manifest: dict[str, Any]) -> list[Finding]:
    ...
    if not isinstance(copy, dict):
        ...
        if "enforceBudgets" in copy and type(copy["enforceBudgets"]) is not bool:
            reject("manifest.copy_budget", "enforceBudgets must be a boolean")
...
def validate_copy(slides: list[Slide], manifest: dict[str, Any]) -> list[Finding]:
    ...
    for slide in slides:
        ...
        if copy.get("enforceBudgets", False) and count_words(title) > budgets["maxTitleWords"]:
            findings.append(
                Finding(
        ...
        if (copy.get("enforceBudgets", False) or "maxWordsPerSlide" in overrides.get(str(slide.number), {})) and slide_words > budgets["maxWordsPerSlide"]:
            findings.append(
                Finding(
        ...
        for shape in counted_shapes:
            ...
            if copy.get("enforceBudgets", False) and words > budgets["maxWordsPerTextShape"]:
                findings.append(
                    Finding(
            ...
            if copy.get("enforceBudgets", False) and shape.paragraphs > budgets["maxParagraphsPerTextShape"]:
                findings.append(
                    Finding(
...
def canonical_runtime_source_state() -> dict[str, Any]:
    files = {
        ...
        for path in sorted(p for p in CANONICAL_RUNTIME_ROOT.rglob("*") if p.suffix in {".mjs", ".py"})
    }
...
def validate_canonical_generation(
    ...
    *,
    ...
    if not isinstance(script_record, dict) or not isinstance(script_record.get("sha256"), str):
        ...
        if ("runtime/generation.mjs" not in script and script_path != CANONICAL_RUNTIME_ROOT / "build-deck.mjs") or required_entrypoint not in script:
                ...
                if isinstance(branches, list):
                    for index, branch in enumerate(branches, start=1):
                        ...
                        if synthesis.get("format") == "thematic-bullets":
                            bullets = branch.get("bullets")
                            if not isinstance(bullets, list) or not bullets or not all(isinstance(b, str) and b.strip() for b in bullets):
                                errors.append(f"slide {number}: branch {index} needs substantive bullets")
                            elif len({semantic_normalize_text(b) for b in bullets}) != len(bullets):
                ...
                'slides': {'type': 'array',
                           ...
                           'items': {'type': 'object',
                                     ...
                                     'required': ['slide',
                                                  ...
                                                  'findings',
                                                  'copyAudit'],
                                     'properties': {'slide': {'type': 'integer', 'minimum': 1},
                                                    'verdict': {'type': 'string',
                                                                'enum': ['accept', 'reject']},
                                                    'summary': {'type': 'string', 'minLength': 1},
                                                    'scores': {'$ref': '#/$defs/scores'},
                                                    'copyAudit': {'$ref': '#/$defs/copyAudit'},
                                                    'findings': {'type': 'array',
                                                                 'items': {'$ref': '#/$defs/finding'}}}}},
                'findings': {'type': 'array', 'items': {'$ref': '#/$defs/finding'}}},
 '$defs': {'copyAudit': {'type': 'object',
                         'additionalProperties': False,
                         'required': ['noRecap', 'recapEvidence', 'insightCount', 'insights'],
                         'properties': {'noRecap': {'type': 'boolean'},
                                        'recapEvidence': {'type': 'string', 'minLength': 1},
                                                     ...
                                                     'items': {'type': 'object',
                                                               ...
                                                               'properties': {'text': {'type': 'string',
                                                                              ...
                                                                              'premises': {'type': 'string',
                                                                                           'minLength': 1},
                                                                              'addedDeduction': {'type': 'string',
                                                                                                 'minLength': 1},
                                                                              'classification': {'type': 'string',
                                                                                                 'enum': ['supported_deduction',
                                                                                                          'recap',
                                                                                                          'unsupported']}}}}}},
           'scores': {'type': 'object',
                      'additionalProperties': False,
                      'required': ['compositionCompleteness',
                                   'messageArchetypeFit',
                                   'hierarchyEconomy',
                                   'evidenceDensity',
                                   'componentFidelity',
                                   ...
                                   'visualConsistency',
                                   'polish'],
                      'properties': {'compositionCompleteness': {'type': 'number',
                                                                 'minimum': 0,
                                                                 'maximum': 100},
                                     'messageArchetypeFit': {'type': 'number', 'minimum': 0, 'maximum': 100},
                                     'hierarchyEconomy': {'type': 'number', 'minimum': 0, 'maximum': 100},
                                     'evidenceDensity': {'type': 'number', 'minimum': 0, 'maximum': 100},
                                     'componentFidelity': {'type': 'number', 'minimum': 0, 'maximum': 100},
                                     'exhibitFinish': {'type': 'number', 'minimum': 0, 'maximum': 100},
                                     'navigationHierarchy': {'type': 'number', 'minimum': 0, 'maximum': 100},
                                     'visualConsistency': {'type': 'number', 'minimum': 0, 'maximum': 100},
                                     'polish': {'type': 'number', 'minimum': 0, 'maximum': 100}}},
           'finding': {'type': 'object',
                       'additionalProperties': False,
                       'required': ['severity', 'code', 'observation', 'reason', 'recommendedChange'],
                       'properties': {'severity': {'type': 'string', 'enum': ['blocker', 'major', 'minor']},
                                      'code': {'type': 'string', 'minLength': 1},
                                      'observation': {'type': 'string', 'minLength': 1},
                                      'reason': {'type': 'string', 'minLength': 1},
                                      'recommendedChange': {'type': 'string', 'minLength': 1}}}}}
VISUAL_ALLOWED_MODELS = tuple(REVIEWER_POLICY["models"])
VISUAL_DEFAULT_MODEL = REVIEWER_POLICY["defaultModel"]
...
Apply the copy owner's executive-summary standalone narrative test at the declared scope, including an ordered multi-page synthesis group. Choose developed bullets, paragraphs, label/body rows, tables or exhibits according to the reasoning; there is no fixed branch count or minimum supports per theme. A single developed support can suffice, and a separate overall-action box is optional when closure is integrated or the purpose does not call for action. Necessary category labels organize evidence without needing to invent a new deduction. A clean exhibit can still fail if a material governing argument, counterargument or change condition remains implicit. Judge substantive completeness and retained qualifications, not word count or table presence alone; do not classify by title keywords. A summary copy-budget override does not authorize smaller type or filler.

Chevron restraint: standalone implication chevrons, including disc-chevrons, should appear at only one or two deliberate emphasis points per deck. Do not require a marker at every insight or rail; compact grouping and reading order are valid. Process chevrons encoding actual stages are a separate use.
...
Contribution review: retain useful interpretation, synthesis and essential qualifications. A supported insight need not add a new deduction. Redundant transcription is advisory unless it materially obscures reasoning. Unsupported claims remain major defects.

Contribution audit: cover visible titles, headings, annotations and supporting sections. Record exact text, role, reading benefit in deletionConsequence, and passes. Necessary navigation and measurement labels are valid. A failed editorial contribution is advisory; identify an explicit material consequence before assigning major severity. Plain explanatory prose may reference an exhibit without a heading or surface.

For EVERY slide, return copyAudit using the supplied schema. The legacy noRecap and addedDeduction fields now describe contribution: a supported synthesis or explanation can use supported_deduction. Recap is advisory; unsupported claims block acceptance. Inspect missing premises and underdeveloped evidence as well as excess. Missing review coverage is an error.
...
Scores are diagnostics. Accept when every slide is covered and there are no unresolved material defects. Pure stylistic preferences use minor severity. A major finding must name an actual factual, readability, geometry, evidence, comparability or missing-argument defect. Return only JSON matching the supplied schema and set rubricVersion to {VISUAL_RUBRIC_VERSION}.
...
def visual_copy_audit_passes(value: Any) -> bool:
    return (not validate_visual_copy_audit(value, "copyAudit")
            and all(item["classification"] != "unsupported" for item in value["insights"]))
...
def validate_visual_cached_report(
    ...
    if required_reasoning_effort and report.get("reasoningEffort", "high") != required_reasoning_effort:
        errors.append("cached visual report reasoning effort does not match the request")
...
def run_visual_model_judge(
    ...
    report = build_visual_report(judgement, pptx, renders, contract, theme_manifest, treatment_ledger, generation_script, model)
    report["reasoningEffort"] = reasoning_effort
    return report


def run_required_copy_check(pptx: Path, scene: Path, contract: Path, report: Path, check: bool = False, render_dir: Path | None = None, model: str = "gpt-5.6-terra", reasoning_effort: str = "high") -> None:
    ...
    command = [node, str(SKILL_ROOT / "runtime/review-deck.mjs"),
...
def visual_cli() -> int:
    ...
    try:
        ...
        if args.check_report:
            ...
            run_required_copy_check(args.pptx, scene, args.contract, Path(copy_path), check=True, render_dir=args.render_dir, model=args.model, reasoning_effort=args.reasoning_effort)
            errors = validate_visual_cached_report(
                report, args.pptx, renders, args.contract, args.theme_manifest,
                args.treatment_ledger, args.generation_script, args.model, args.reasoning_effort,
            )
            if errors:
                print("\n".join(f"ERROR: {error}" for error in errors), file=sys.stderr)
                return 1
            ...
            run_required_copy_check(args.pptx, scene, args.contract, copy_path, render_dir=args.render_dir, model=args.model, reasoning_effort=args.reasoning_effort)
            report = run_visual_model_judge(
                args.pptx, renders, args.contract, args.theme_manifest,
                args.treatment_ledger, args.generation_script, args.model, args.timeout_seconds, args.reasoning_effort,
            )
...
CONSISTENCY_SCHEMA = {'$schema': 'https://json-schema.org/draft/2020-12/schema',
 'type': 'object',
 'additionalProperties': False,
 'required': ['rubricVersion',
              'verdict',
              'summary',
              'deckScores',
              'slideCoverage',
              'comparisonGroups',
              'findings'],
 'properties': {'rubricVersion': {'type': 'string'},
                'verdict': {'type': 'string', 'enum': ['accept', 'reject']},
                'summary': {'type': 'string', 'minLength': 1},
                'deckScores': {'type': 'object',
                               'additionalProperties': False,
                               'required': ['visualSystemCoherence',
                               ...
                               'properties': {'visualSystemCoherence': {'type': 'number',
                                                                        'minimum': 0,
                                                                        'maximum': 100},
                                              'componentVariantContinuity': {'type': 'number',
                                                                             'minimum': 0,
                                                                             'maximum': 100},
                                              'trackerContinuity': {'type': 'number',
                                                                    'minimum': 0,
                                                                    'maximum': 100},
                                              'titleAndGridContinuity': {'type': 'number',
                                                                         'minimum': 0,
                                                                         'maximum': 100},
                                              'legendAndExhibitConsistency': {'type': 'number',
                                                                              'minimum': 0,
                                                                              'maximum': 100},
                                              'densityRhythm': {'type': 'number',
                                                                'minimum': 0,
                                                                'maximum': 100},
                                              'semanticColourConsistency': {'type': 'number',
                                                                            'minimum': 0,
                                                                            'maximum': 100}}},
                'slideCoverage': {'type': 'array', 'items': {'type': 'integer', 'minimum': 1}},
                             ...
                             'items': {'type': 'object',
                                       ...
                                       'properties': {'severity': {'type': 'string',
                                                      ...
                                                      'recommendedChange': {'type': 'string',
                                                                            'minLength': 1}}}}}}
CONSISTENCY_ALLOWED_MODELS = VISUAL_ALLOWED_MODELS
CONSISTENCY_DEFAULT_MODEL = "gpt-5.6-luna"
CONSISTENCY_RUBRIC_VERSION = "5"
...
Score the deck from 0 to 100 on exactly these dimensions: {', '.join(CONSISTENCY_SCORE_NAMES)} as diagnostics. Accept with complete coverage and no material defects. Editorial preferences are minor. Include comparison groups for genuinely recurring constructions across at least two slides. Return only JSON matching the supplied schema and rubricVersion {CONSISTENCY_RUBRIC_VERSION}.
...
def validate_consistency_cached_report(
    ...
    if required_reasoning_effort and report.get("reasoningEffort", "high") != required_reasoning_effort:
        errors.append("cached consistency report reasoning effort does not match the request")
...
def run_consistency_model_judge(
    ...
    with tempfile.TemporaryDirectory(prefix="professional-slides-consistency-eval-") as directory:
        schema_path = Path(directory) / "schema.json"
        schema_path.write_text(json.dumps(CONSISTENCY_SCHEMA), encoding="utf-8")
        raw_output = Path(directory) / "judgement.json"
        command = [
            "codex", "exec", "--model", model,
            "-c", f'model_reasoning_effort="{reasoning_effort}"',
        ...
        judgement = json.loads(raw_output.read_text(encoding="utf-8"))
    report = build_consistency_report(judgement, pptx, renders, contract, theme_manifest, treatment_ledger, generation_script, model)
    report["reasoningEffort"] = reasoning_effort
    return report
