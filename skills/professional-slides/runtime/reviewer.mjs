...
export function reviewPrompt(packet) {
  ...
  return `You are reviewing a consulting deck against the client's brief. Read the spreads and the montage; read the text. Judge it the way an engagement manager would the night before a steering committee.
...
VISUAL REVIEW. Read the deck through its spreads (rendered/spread-*.png, four pages to a sheet at reading size) and the montage; open a page at full size only where a spread shows a possible defect or a decisive number needs checking. Keep the review to one pass and at most twenty findings, the most severe first. Follow the semantic checks in references/design.md: coherent argument and counts; reconciled totals, periods, sample membership and durations; scoped comparisons, non-causal wording unless supported, and reversal conditions that affect the named option; focus that supports the claim; appropriate table category/dimension grammar; one chart heading owner; consistent qualifiers; vertically balanced sparse groups; meaningful arrows and rules; and cross-slide consistency. Neutral charts, joined verdicts and optional commentary are valid where they serve the page.
