...
- **Full-height reasoning split:** when a boundary is useful across the complete list field, use `section-boundary` with `variant="inference-chevron"`. Its quiet dashed divider stops above and below the centered shaftless chevron. Size the boundary to the actual bullet-content extent; exclude headings, the terminal insight and footer. Do not stretch a short argument solely to fill the slide.
