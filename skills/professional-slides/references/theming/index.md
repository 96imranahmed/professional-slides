...
Choose and explicitly declare density per coherent slide family. A deck may have an `executive` main story and a `pre-read` or `appendix` analytical family. The planner respects an explicit profile; it never silently shrinks that choice because a row or mark count crossed a threshold. Without an explicit choice, capacity counts supply a conservative default, recorded as a recommendation rather than proof of fit. Measure actual content at its allocated width and inspect the exported artifact. Retain larger type when the complete exhibit fits; if it does not, remove duplication, rebalance geometry or choose a denser profile for the whole family. Profile changes affect every semantic type role together. A table's internal `compact` or `dense` rhythm must also be chosen explicitly; the default body does not become smaller merely because rows were added. No density selection exempts real overflow or unreadable text.
