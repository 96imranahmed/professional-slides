...
| Words of page text | 185 (p20 104, p80 278) | 120 (p20 99, p80 172) |
| Discrete text blocks | 142 | 95 |
| Numeric tokens | 17 | 23 |
| Drawn objects (marks, rules, brackets) | 29 | 31 |
| Pages carrying 200+ words | 42% | 6% |
...
It resolves in three steps: the deck's own `weight`, then the `weight` in the house profile a `template` produced, then the defaults for the deck's `fill` (full 150 / 0.68 / 0.60, balanced 105 / 0.55 / 0.52, airy off). The balanced floor is the reference corpus's own lower quintile: below it a page carries less than the thinnest fifth of published client pages. Every number is a floor, never a ceiling: the ceiling on prose is the `WORDS` gate and it has not moved. A catalogue of components or chart types declares `fill: "airy"` and the floors switch off - a page that exists to show one encoding is not carrying an argument.
