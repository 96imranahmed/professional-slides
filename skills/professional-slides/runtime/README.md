...
`chart-title` owns graph headings. Its `underlined` default shares the section-heading typography and eight-pixel rule gap, including when a unit is present. A short unit renders on the same line as a measured heading as a separate light-grey body-sized object; it falls below only when the combined title cannot fit. The explicit `unit` variant keeps that two-level treatment without a rule. A chart's optional `heading`, `unit`, and `titleVariant` props, standalone chart titles, and every `chart-group` child use this same owner. Keep periods in concise headings rather than packing them into unit strings.
