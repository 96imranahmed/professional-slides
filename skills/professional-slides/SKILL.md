...
- **The basis.** A footnote that says what is included, what is excluded, and as at when. It costs a line and it is the difference between a claim and an assertion. `note` takes a list as well as a line - `note: ["Excludes the 2019 disposal", "FY22 basis; figures may not sum"]` prints as numbered notes under the page, which is what a superscript in a label or heading ("Revenue¹") points at. The reference pages carry two to four.
