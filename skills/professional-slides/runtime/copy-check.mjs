...
export const COPY_CHECK_VERSION = '4';
...
export function buildCopyInventory(scene, contract = {}) {
  ...
  return {version:COPY_CHECK_VERSION, outline:slides.map(s=>({slide:s.slide,title:s.title,kind:s.pageKind})), question:contract.mainQuestion || '', governingAnswer:contract.governingAnswer || '', slides};
}
...
${JSON.stringify({outline:inventory.outline,question:inventory.question,governingAnswer:inventory.governingAnswer,slides})}`;
}
