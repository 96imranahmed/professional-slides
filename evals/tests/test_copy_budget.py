...
""")
    ...
    def test_override_is_explicit_scoped_and_still_enforced(self):
        result = run_node("""
import assert from 'node:assert/strict';
import {validateSlidePlan,planSlide,planDeck} from './skills/professional-slides/runtime/planner.mjs';
...
assert.doesNotThrow(()=>validateSlidePlan(plan));
const approved={...plan,copyBudget:{maxWordsPerSlide:160,rationale:'Reference-led standalone narrative'}};
assert.equal(planSlide(approved).decision.content.countedWords,100);
assert.equal(planSlide(approved).decision.content.budget,160);
assert.equal(plan.density,'pre-read');
assert.equal(plan.copyBudget,undefined);
assert.doesNotThrow(()=>validateSlidePlan(plan));
assert.throws(()=>planDeck({id:"budget",slides:[{...approved,copyBudget:{...approved.copyBudget,maxWordsPerSlide:99}}]}),/visible copy exceeds explicit budget 99/);
...
assert.equal(validateSlidePlan(complete).countedWords,107);
