...
Every analytical row must explicitly name its key insight and link the underlying data or specific source evidence. Here `Key insight` records what the page establishes for the audience; it is planning metadata, not a requirement to render a separate insight box. For a descriptive evidence page, state the supported finding and mark `no detached insight` when no further deduction is warranted. Distinguish the exact proposed visible copy from non-rendered evidence assignments and rationale. Link both the original source and the matched dataset or calculation when the conclusion is derived; a source list at the end is insufficient.
...
Make the implementation choice concrete enough to avoid rediscovery: name the exact registered component and variant, the actual categories or rows, and a short reason tied to their reading task. For example, a four-option comparison can use one table with action, annual benefit, implementation horizon and dependency columns because the reader must compare those fields together. A series of four sparse option cards would obscure that comparison. Keep this rationale in the design cell, never on the canvas.

Resolve nested choices as well as the main exhibit. Check the registry's accepted variant IDs and required props rather than assuming a `default` variant exists. Compile the complete planned content at its intended family density before an expensive export to catch impossible chart frames, wrapped table rows and missing component fields. This is a feasibility check, not rendered acceptance; preserve the first plan and any concrete failure when evaluating authoring quality.
