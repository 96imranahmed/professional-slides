...
The `quantitative-scale` variant accepts `scale: { domain: [min, max], unit, palette, decimals }` and top placement. It uses eleven shared heat-scale bins with measured exact endpoint labels; map values and legend bins resolve through the same scale helper. Increasing finite bounds and explicit units are required. It rejects a legend too narrow or short for its labels, and does not accept categorical items or right placement as substitutes for a quantitative scale.
