...
- Use when one reconciled whole has two to five parts and approximate share is sufficient.
- Why: the circular form makes a dominant share or simple concentration immediately recognizable.
- Action title: state the dominant share, balance, or concentration; do not use a generic mix label.
...
The core pie is one circle whose angle encodes share of one whole. Start at twelve o'clock, order slices by value unless a natural order matters, and proceed consistently around the circle. Labels that identify a slice directly with its category and value must pass the shared [direct-label gate](index.md#direct-label-gate). Internal percentages are value labels rather than direct labels when a legend supplies category identity. Never use perspective, gradients, exploded slices, or several nearly identical hues.

## Registered variants

- `legend-top-right` (default): select the shared [top-right legend](../components/chart-legends.md#registered-variants), let that legend supply category identity, keep percentage value labels inside, and omit outside category labels. Centre the circle horizontally in the full section and vertically in the remaining plot area below the legend band.
...
- `shared-legend`: omit local category labels and legends; use the [chart-group contract](../components/chart-legends.md#coordinated-chart-groups) for shared mapping and layout. Prefer 100% stacked bars for precise comparisons across totals.

Both `chart.pie` and `chart.donut` expose these three runtime variants. The golden set renders each separately in the canonical McKinsey palette; fast contract tests retain cross-palette coverage. A donut centre remains empty unless a separately registered, tested component supplies meaningful content.
...
Verify the whole and denominator are explicit. Shares reconcile to 100% after rounding. The focal share needs no exploded slice. Internal percentages and any outside labels fit their declared variant; legends pass the [shared acceptance check](../components/chart-legends.md#acceptance-check). A sorted or 100% stacked bar would not make the comparison clearer.
