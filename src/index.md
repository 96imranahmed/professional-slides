...
2. Read storylining and construct the problem hypothesis tree, governing thought, mapped slide-by-slide dot-dash, chapter sequence, action-title spine, and tracker map. Compile them into the [`pre-authoring deck contract`](storylining/pre-authoring-contract.md), validate it, and do not create a slide document until the complete dot-dash passes narrative QA, receives the explicit owner approval required by its approval gate, and the contract succeeds.
...
1. Inspect the complete existing deck before editing: read the action-title spine, render all pages, identify the active master and layouts, inventory recurring components, and distinguish deliberate exceptions from broken consistency. Create an as-is dot-dash with exactly one sequenced dot for every source slide, compile it as `existing_deck_revision` in the [`pre-authoring deck contract`](storylining/pre-authoring-contract.md), and validate it before the first mutation.
...
4. Do not force-add an executive summary or other new-deck structural page to an existing deck. Record a missing executive summary as a recommendation and implement it only when the requested scope or explicit owner approval authorizes the change. Update the hypothesis tree, target dot-dash, pre-authoring contract, and action-title spine when the change alters the argument, slide order, or evidence. Recompute chapter boundaries, page numbers, trackers, cross-references, appendix references, and transitions after slides are added, removed, or reordered.
