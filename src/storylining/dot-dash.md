...
For a new deck, represent every planned slide in production order with exactly one dot, including every cover, executive synthesis, contents, chapter transition, analytical page, decision close, and appendix page required by the approved story. For an existing-deck revision, first represent every source slide in its current production order with exactly one as-is dot, including structural gaps and weak pages; do not manufacture a missing executive synthesis or other new page in the inventory. A dot cannot stand for a chapter or an unspecified run of slides. Split one assertion into several sequenced dots when its proofs require several pages, and split any dot that contains two independent primary claims. Write an analytical dot as a provisional action title and label every dot with a dot ID, its exact approved chapter or tracker item, and the hypothesis-tree node or nodes it resolves; use `NAV` for a purely structural page.
...
For a new deck, review and revise the complete outline with the relevant decision owners before creating a deck document, layout, or exhibit. If the agent runtime provides `gather feedback` or an equivalent interactive review capability, present the reviewable dot-dash through it, ask the owner to approve or request revisions, apply every accepted change, and resubmit the complete revised artifact until the owner explicitly approves it. If that capability is unavailable, provide the reviewable artifact in the available channel and request the same explicit approval. Silence, lack of edits, approval of the brief, or approval of an earlier version does not count as approval of the current dot-dash.

For an existing-deck revision, complete and preserve the as-is dot-dash before the first mutation. Do not require separate owner approval merely to record the current slide sequence for a narrow edit. If the work adds, removes, reorders, or materially repurposes slides, present the proposed target dot-dash and obtain explicit approval before making that structural change. If the existing deck lacks an executive synthesis, recommend one but do not insert it unless the owner authorizes the addition.

For a new deck, do not create a PowerPoint file, native Google Slides document, ghost deck, or other slide document until the explicitly approved dot-dash is preserved as a reviewable artifact and the pre-authoring contract validates. For an existing-deck revision, do not mutate the source deck until the complete as-is dot-dash and contract are preserved; approval is required only for a proposed structural change. A storyboard, page list, set of action titles, or agent self-check does not replace the applicable gate. When production changes the thesis, chapter order, page sequence, action-title meaning, or evidence role, update and reapprove the target dot-dash before changing the deck.
...
- A new deck's current complete outline has explicit owner approval, obtained through the runtime's interactive feedback capability when available, before any slide document is created; an existing-deck revision has a complete as-is dot-dash before the first mutation and explicit approval only for target structural changes.
- The dot-dash is compiled into a valid pre-authoring contract at the stage required by the workflow mode.
