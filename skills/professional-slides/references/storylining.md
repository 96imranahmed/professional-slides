...
- **Shape.** The page architecture: `exhibit-left`, `exhibit-right`, `exhibit-top`, `hero-number`, `two-up`, `split-tone`, `grid`, `exhibit-full`, `picture-hero`, `picture-pair`, `picture-strip`, `table-halves`, `text`. A deck built from two shapes reads as one page repeated; the plan gates measure the spread as entropy and cannot measure it at all when this column is blank. Where a run of pages is one template on purpose — six use-cases, eight market profiles — name the run in this column as `series: <name>` and the gates count it once instead of punishing a deliberate decision.
