...
| Page types | Every page authored as a type with its form, commentary placement and close chosen; the variety contract passes | `<id>.pages.json` compiled to `.deck.json` and `.plan.json` | [Page types](references/page-types.md) |
...
```bash
node runtime/author-deck.mjs <id>.pages.json     # page types -> <id>.deck.json + <id>.plan.json; refuses a repeated deck
