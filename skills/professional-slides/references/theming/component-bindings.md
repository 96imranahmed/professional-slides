...
| table-cell status and comparison indicator | Completion, status, heatmap, rating, and legend roles; `--table-cell-binary-line` | [Comparison owner](../components/comparison-indicators.md); cells and legend share the resolved scale. Binary check, cross and missing strokes bind to `line.standard`; row separators retain hairline weight. |
