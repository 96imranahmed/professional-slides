...
7. Pass the [analytical substance check](references/components/copy.md#analytical-substance) before choosing layout, then compile every planned slide through the [content and composition planner](runtime/README.md#content-planning). Every content item must state its job and use a registered component.
...
- Keep the [cover](references/components/index.md#deck-cover) title very short and its optional subtitle to a short identity such as the company name; do not place a thesis or provenance paragraph beneath it.
...
- When the right-hand conclusion follows from the left-hand evidence, explicitly include the shared [inference boundary](references/components/guidelines.md#split-section-relationships) in the content plan and verify its arrow in the render.
