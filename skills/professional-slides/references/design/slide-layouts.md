...
When a side section expands a specific period, category or segment selected in the main chart, it is a companion analytical exhibit. Give that deep dive a substantive heading and an underline aligned with the main chart’s heading rule, even if the deep dive uses metrics rather than another chart. For example, “2023 releases: lifetime gross” names the selected-year detail and warrants a rule. A standalone implication, scope note or recommendation is a different role and does not inherit that heading or rule requirement. Decide from the relationship and analytical job, not a blanket rule for every RHS or every non-chart component.
