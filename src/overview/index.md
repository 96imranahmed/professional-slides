...
`SKILL.md` is the base entrypoint and router. Everything below `src/` belongs
to a named subsystem; the source root intentionally contains no loose files.
Read this overview when changing the package structure or deciding
...
Platform implementation does not belong under `src/`. PowerPoint and
...
2. Do not add loose files directly under `src/`; create or reuse the
...
6. Keep platform-neutral semantics in `src/` and host/API mechanics in
   `tools/`.
