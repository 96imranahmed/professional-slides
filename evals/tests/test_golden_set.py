...
const stacked=title.render({id:'title',frame:{x:60,y:180,width:230,height:140},props:{heading:'Reported income adjusted to normalized income',unit:'$B'}}).nodes;
assert.equal(stacked.find(n=>n.role==='chart-unit').data.chartUnitPlacement,'stacked');
assert.ok(stacked.find(n=>n.role==='chart-unit').frame.y>=stacked.find(n=>n.role==='section-heading').frame.y+stacked.find(n=>n.role==='section-heading').frame.height);
assert.equal(stacked.filter(n=>n.role==='section-heading-rule').length,1);
...
assert.equal(chart.filter(n=>n.role==='section-heading-rule').length,1);
...
assert.equal(audit.slides,audit.componentSlides+audit.layouts+audit.standards);
assert.ok(audit.slides<coverage.length+audit.layouts+audit.standards);
console.log(JSON.stringify(audit));
""")
        self.assertEqual(result["components"], 64)
        self.assertEqual(result["standards"], 27)
