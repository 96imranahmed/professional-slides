...
## Turn the evidence into a first draft

Work in this order before polishing titles or selecting layouts:

1. Resolve the communication job and the current decision stage. Identify what the evidence permits the audience to conclude now, what needs further design or testing, and what is outside the requested decision.
2. Read the complete evidence base and build its [evidence ledger](references/storylining/index.md#organize-the-evidence-before-writing-titles). Preserve measurement bases, scenario assumptions, source precision and conflicting inputs. Form the answer from that ledger; do not write an attractive thesis and search for supporting fragments.
3. Group evidence into distinct tests of the answer. Put comparable options, segments or scenarios together; give a separate page to a developed test, mechanism or decomposition. One main claim can require a dense portfolio, multiple related exhibits or several linked premises. It does not mean one source fact per slide.
4. Draft the complete argument as dot-dash, including the strongest countercase and the conditions on the answer. Assign each fact one primary place in the body. The summary synthesizes those arguments; a secondary box on the same page must contribute something further.
5. Choose the [exhibit and its capacity](references/design/index.md#allocate-evidence-before-geometry) from the actual rows, categories, periods and text. Resolve registered variants and their required inputs before export. Preserve comparable page families and use measured content to allocate space. Declare the page density explicitly and retain the normal table body unless the whole table family needs a compact treatment. More rows alone do not justify smaller type when the evidence fits at a readable size.
6. Write the visible copy by [information role](references/components/copy.md#write-evidence-interpretation-and-action-once), then run the criticality and claim-to-proof checks. First presentation of necessary evidence is valid without an invented deduction. A deduction needs additional reasoning; an action needs a supported decision and authority.

For source-deck development, complete a [slide-by-slide structural comparison](references/evaluation/index.md#comparison-with-a-reference-deck). Carry each source exhibit's strongest analytical device, joint comparisons and readable evidence density into the proposed design. Audit the sequence for distinct reading tasks rather than swapping chart types inside one repeated layout. Give each analytical exhibit [one heading owner](references/design/slide-layouts.md#one-heading-owner-per-analytical-exhibit); a chart's parent container must not add a redundant title above its measure heading.

Use the existing three deck templates when their decision fits; otherwise use the generic storylining procedure. Client, industry and consulting-firm names do not create new content types. Extend a canonical component only when a recurring evidence relationship cannot be expressed faithfully with its existing inputs or variants.
...
2. Organize the evidence and build the problem logic using the first-draft procedure above: a hypothesis tree for a decision under uncertainty, or a concept/dependency map for an explanatory brief, following [Storylining](references/storylining/index.md).
...
- One slide, one main claim, one dominant exhibit. Keep the linked evidence needed to prove that claim together; a composite exhibit or dense comparison can be the dominant exhibit.
...
- Executive summaries need multiple substantive bullets under each titled section, not one compressed point per theme. Apply the [executive-summary completeness contract](references/components/copy.md#executive-summary-narrative), keep shared body type, and close with the supported recommendation or explanatory conclusion and its material condition. Match the communication job and decision stage.
...
- Executive summaries use substantive headings and evidence-dense developed arguments. A decision brief needs an actual recommendation within the current decision stage; an explanatory brief needs a supported synthesis. Never print chapter numbers or navigation labels as summary headings. Store semantic theme-to-body mapping in the plan; themes may consolidate related chapters. A generic instruction to compare or verify is not the recommendation.
