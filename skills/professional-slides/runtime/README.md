...
`build-deck.mjs spec.json output-directory [--preflight]` consumes one validated deck specification and derives planning/acceptance artifacts. `deliver-deck.mjs spec.json output-directory` adds package/provenance checks and coordinated rendered review. Both ship inside the installed skill. See [production](../references/tools/production.md).

`review-deck.mjs` batches exact rendered slides, caches independently verified slide judgements by local evidence and declared dependencies, and rebinds complete coverage to the current PPTX. `production-policy.mjs` owns material severity, concurrency and repair limits. Generation produces PNGs during its existing Artifact Tool import. `benchmark.mjs` summarizes measured runs; `learning-corpus.mjs` validates authorized hash-bound examples without manufacturing human labels.
...
`paragraph.props.text` requires a non-empty string. Keep dimensions in the item size or frame; use named arguments in authoring helpers so a missing ID cannot shift a height into the text position. Reconcile compiled visible text with the storyboard before export, since type validation alone cannot detect an omitted or wrong sentence.

An explicit user-imposed `copyBudget: { maxWordsPerSlide, rationale }` constrains only that slide; default density counts are advisory. Both fields are required; the planner records the effective and default limits in its decision. Copy the effective limit into the exported-file acceptance manifest. This changes neither type tokens nor geometry and never certifies narrative quality; follow the [executive-summary copy owner](../references/components/copy.md#executive-summary-narrative).
...
`tracker-page` is the reusable full navigation state. It accepts one stable `trackerId`, three to eleven `{ id, label }` items, an optional `selectedId`, and registered sequential-circles or split-contents variants in light and dark modes. The light-only `text-agenda` layout supports three to six single-line labels with equal-emphasis overview and selected-section states, large vertically stacked type, and no visible markers or heading. It preserves label frames across states and uses weight plus neutral colour for selection. Split contents also registers regular and long density states. Its selected row defaults to `selectionTreatment: "tint"`, keeping dark text on a light theme-bound surface; `"inverse"` is the explicit stronger alternative and inverts the circular marker outline as well as its label. `tracker-label` is the associated analytical-page state with compact label, breadcrumb, and number-strip variants. `planSlide({ tracker })` renders that compact state above the action title; use the same item objects and selected id as the full page. The runtime rejects stale or unmatched selections, preserves peer geometry, and records tracker and section ids on native nodes for continuity checks.

For a full tracker in a content deck, use `kind: "tracker"`, the selected chapter label as `title`, and the registered tracker props in `trackerPage`. The planner renders the full navigation state with page furniture and no analytical action-title, so the selected label appears once. Use short stable item IDs for circle markers.
...
Paragraphs and tables freeze measured line breaks for both adapters. The canonical [`typed table`](../references/charts/heatmap-table.md) uses `tables.mjs` for text, bullets, categories, highlights, exact numbers, binary indicators, Harvey balls, heatmaps, bars, and implication columns. Rows and columns are entirely data-driven rather than capped by a business-specific schema. Columns supply defaults; cells may override them, and category cells may span grouped rows. `treatment: "open"` is the default header style. Table density defaults to `body` at every cardinality; select `compact` or `dense` explicitly for a coherent table family after measuring content. Numeric cells default to centred theme-bound circles and accept `numberDisplay: "plain"` for ordinary right-aligned values. Named scales generate shared legends below the exhibit.

`planSlide` preserves an explicit `density` for the page family and uses capacity recommendations only when it is omitted. Four-branch or six-leaf trees, nine to twelve chart marks, and tables beyond five rows or four columns suggest `pre-read`; larger extents suggest `appendix` at the registered thresholds. The decision records `requested`, `recommended`, `resolved`, `selection` (`explicit` or `capacity-default`) and capacity reasons. These counts do not establish fit: intrinsic measurements and rendered checks still reject overflow. A selected profile applies to every semantic type role on the page. Table `props.density` separately selects its internal rhythm and defaults to `body`; neither level silently shrinks an explicit choice.
...
`chart-title` owns graph headings. Its `underlined` default shares the section-heading typography and eight-pixel rule gap, including when a unit is present. A short unit renders on the same line as a measured heading as a separate light-grey object at the heading's font size; it falls below only when the combined title cannot fit. The explicit `unit` variant keeps that weight-and-colour distinction without a rule. A chart's optional `heading`, `unit`, and `titleVariant` props, standalone chart titles, and every `chart-group` child use this same owner. Keep periods in concise headings rather than packing them into unit strings.

`valueFormat: { decimals, prefix, suffix, grouping }` formats visible values without rounding the source data or mark geometry. Set `grouping: true` for thousands separators; fractional precision, signs and compact-unit scaling are preserved. Axis labels retain fractional ticks. Waterfalls support signed `yMin`/`yMax` domains and negative closing totals; lines accept `directLabels: "end"` to replace a legend with semibold series-and-value endpoints. Both have isolated golden variants. Speaker notes pass through the planner, canonical scene and PowerPoint adapter unchanged; use them for derivations and methodology, following the [copy owner](../references/components/copy.md#insight-versus-speaker-notes).
...
Bar and column charts accept one `highlights` entry. `style: "bar"` focuses an exact category in an unstacked one-series chart using `color.componentPrimary` against `color.chartComparator` light-grey peers. `style: "region-box"` draws a primary outline around a complete category, while `style: "region-tint"` uses a light neutral field. Both region styles reserve breathing room beyond the plot rails and keep the category label clear. A missing style retains neutral-tint behavior.

For two unstacked series, `focusSeries: "Actual"` assigns primary to that exact series and comparator grey to the other, including legend swatches. It rejects unknown keys, more than two series, stacked charts, and conflicting `colorIndices`. Category and series focus keys survive reordering. Without a declared focus or explicit colour mapping, legacy two-mark/two-series inputs use primary for the first mark/series and grey for the second; authors must select the meaningful focus explicitly rather than relying on that positional fallback. `colorIndices` preserves explicit categorical mappings, including shared chart-group legends. See the [authoring colour contract](../references/charts/index.md#focus-and-comparator-colours). Example data never enters production rendering implicitly.

Compatible categorical, line, waterfall, scatter, and combo charts accept `changeAnnotations`. Each entry declares exact `start` and `end` category keys, optional series keys, concise `text`, and `style: "arrow" | "bracket" | "construction"`. Arrow is directional A-to-B movement, bracket is a selected interval, and construction is a reconciled opening-to-closing bridge. Column, stacked-column, line, waterfall, and combo charts may also use `annotationRail: { rows: [{ label, items: [{ category, text }] }] }`. Each row has a left-hand measure label and each bubble has one value; the legacy `{ label, items }` single-row form remains supported. Row height and left-label width are reserved before plotting. Pie, donut, scatter, and horizontal category axes reject that rail. Datapoint labels and legends use `type.chartLabel`; callouts and change labels use `type.chartAnnotation`. Both roles resolve to `type.body` by default, while their regular or semibold weights remain distinct. Every label uses the active component primary rather than a local accent.
