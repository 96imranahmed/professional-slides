...
Count, placement, treatment, arrangement, and attribution placement are separate inputs. The `quotes` array determines count. `placement` declares whether the cluster is dominant or sectional. `treatment` selects `contained`, `callout`, or `speech-bubble`. `arrangement: "staggered"` selects the three-voice diagonal composition; other combinations use a grid. `attributionPlacement` selects `inside`, `below-center`, or `below-left` within the compatible treatment.
...
- Use one page-level source treatment. Source references may connect individual statements to numbered source entries, but each quote item does not receive a duplicate source block.
