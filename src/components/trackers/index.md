...
```css
:root { --primary: #14293d; --accent: #2f7df6; --muted: #7b8794; --surface: #eef4fb; --paper: #fff; }
.slide { position: relative; width: 1280px; height: 720px; padding: 42px 64px; background: var(--paper); color: var(--primary); font-family: Arial, sans-serif; }
.tracker-item { color: var(--muted); }
.tracker-item[aria-current="step"] { color: var(--primary); font-weight: 700; }
.running-label { color: var(--accent); font-size: 15px; font-weight: 700; }
.action-title { max-width: 1040px; margin: 14px 0 0; font-size: 38px; line-height: 1.08; }
.tracker--top { display: grid; grid-template-columns: repeat(4, 1fr); width: 620px; border-bottom: 1px solid #d7dee7; }
.tracker--top .tracker-item { padding: 0 0 11px; text-align: left; }
.tracker--top .tracker-item[aria-current="step"] { border-bottom: 4px solid var(--accent); }
.sidebar-layout { display: grid; grid-template-columns: 320px 1fr; padding: 0; }
.tracker--sidebar { padding: 54px 30px; background: var(--surface); }
.tracker--sidebar .tracker-item { display: block; padding: 13px 14px; }
.tracker--sidebar .tracker-item[aria-current="step"] { background: #dce9fb; border-left: 5px solid var(--accent); }
...
.tracker--grid .tracker-item[aria-current="step"] { border-color: var(--accent); }
...
.tracker--badged-list .tracker-item[aria-current="step"] { background: #dce9fb; outline: 2px solid var(--accent); }
...
Carry the exact label through the contents page, chapter heading, persistent rail or badge, running label above the action title, and any cross-reference. Do not synonymize, abbreviate, or renumber it locally. If a compact form is necessary, approve it in the tracker record and use that same form everywhere the compact variant appears.
...
<section class="slide">
  <nav class="tracker--top" aria-label="Deck chapters">
    <span class="tracker-item" aria-current="step">Case</span>
    <span class="tracker-item">Business</span>
    <span class="tracker-item">Risk</span>
    <span class="tracker-item">Decision</span>
  </nav>
  <h1 class="action-title">The strategic fit warrants diligence—not approval</h1>
</section>
```
...
Highlight the active item with one restrained state such as a light surface, a small edge marker, a short rule, or accent-coloured type. Keep inactive labels readable but subordinate. Repeat the exact active label as a running label above the action title so the page remains identifiable when the sidebar is cropped, printed, or viewed as a thumbnail.
...
- any optional running labels above action titles match the active tracker item exactly or use its one approved compact form, and no isolated running label is counted as the tracker system;
