...
`pictures` carries one to five labelled photographs. A `metrics` strip uses `metricsPosition: "top" | "bottom"`; omit it when it duplicates the argument. `paragraphs` carries body prose; on a text page with no points it flows from the top as a report page does, opening a second column past about 170 words and a third past about 340, and `textColumns: 1 | 2 | 3` fixes the count. `pointsHeading: false` omits a commentary heading, while `pointsAlign: "middle"` centres the group; headed commentary normally begins at the top. An unheaded column that fills two thirds of its track or less centres on the exhibit without being asked, because a short column hugging the top of a tall track leaves a hole at the foot of the page; `pointsAlign` overrides either way. `insights` holds a reading followed by its consequence, while `insight` holds a single developed implication.
