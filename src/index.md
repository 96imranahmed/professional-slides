...
This index routes the instructional content under `src/`. Each concept has one owner so the guidance remains mutually exclusive and collectively complete.
...
For a new or materially revised deck, read storylining and design, then route only to the relevant slide types, charts, design mode, components, and target-platform tools. For a narrow repair, read this index and only the owner of the affected behavior.
