...
**One deck, several treatments.** A verdict column has three correct drawings, and a deck that always takes the first has made one decision and repeated it: nine of fourteen tables in a cold run drew the gutter, and the reader stopped seeing it. The three are the **gutter** (`implication: true`), the **tinted conclusion cell** (the last column's cells as `{ "type": "highlight", "text": … }`, no gutter) and **filled category cells** down the first column (`{ "type": "category" }` on the column and its cells, the verdict left plain). Where the columns are written as plain strings the composer rotates the three for you, taking whichever the last few tables did not; where a column is written as an object the author has decided and the composer leaves it alone. `DECK_CRAFT` reports a deck that puts the same device on more than 60% of its tables.
